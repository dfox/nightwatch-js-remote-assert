  ...
  'Notes can be loaded using fixture' : (browser) => {
      browser.loadTestData('notes.json', (notes) => {
          browser
              .url('http://localhost:8082')
              .waitForElementVisible('body', 1000)
              .setValue('input[id=note-name]', notes.fixture.name)
              .click('button[id=load-button]')
              .pause(1000)
              .assert.containsText('#status', 'Error loading note: Not Found')
              .assert.valueContains('textarea[id=note-content]', '')
              .remoteFixture('co.cantina.junit.http.example.ExampleTest', 'createNote', () => {
                  browser
                      .click('button[id=load-button]')
                      .pause(1000)
                      .assert.containsText('#status', 'Loaded note: ' + notes.fixture.name)
                      .assert.valueContains('textarea[id=note-content]', notes.fixture.contents)
                      .end();
              });
      });
  }
